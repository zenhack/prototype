...
class Model(object):
    ...
    def run(self, framespec=None, stylesheet=None):
        def default_filename(obj, ext):
            return type(obj).__name__.lower() + '.' + ext

        elm.init()
        my_frame = frame.from_file(framespec or default_filename(self, 'xml'))
        css = CssParser(stylesheet or default_filename(self, 'css'))
        css.attach(my_frame)
        w = my_frame.widget(self)
        elm.run()
        elm.shutdown()
